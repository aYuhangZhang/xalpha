...
def test_index():
    assert (
        round(zzhb.price[zzhb.price["date"] ==
                         "2012-02-01"].iloc[0].totvalue, 3)
        == 961.406
    )
    assert (
        round(zzhb.price[zzhb.price["date"] ==
                         "2015-02-02"].iloc[0].netvalue, 2)
        == 1.62
    )
    assert zzhb.name == "中证环保"
    assert zzhb.shengou(100, "2018-01-02")[2] == 55.24
    assert zzhb.shuhui(100, "2016-01-01",
                       [[pd.Timestamp("2017-01-03"), 200]])[2] == 0
    zzhb.info()
    zzhb.ma(window=10)
    zzhb.md()
...
def test_fund():
    ...
    assert (
        float(hs300.special[hs300.special["date"]
                            == "2017-08-04"]["comment"]) == 0.19
    )
    hs300.rate = 0.12
...
def test_mfundinfo():
    ...
    with pytest.raises(FundTypeError) as excinfo:
        xa.fundinfo("001211")
    assert str(
        excinfo.value) == "This code seems to be a mfund, use mfundinfo instead"
...
def test_vinfo():
    ...
    hs300.info()
    st = pd.DataFrame(
        {"date": ["20200101", "20200203"], "SH000300": [200, -100]})
