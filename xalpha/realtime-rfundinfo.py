...
# logger.add('checkDuplicateBuy.txt', format="{time} {level} {message}")
...
def check_duplicate_buy(fundNum, price_scope_max, price_scope_min):
    ...
    read = xa.record(path)
    history_transaction = xa.fundinfo(fundNum)
    ...
    for i in range(0,len(transaction_record)):
        ...
        history_price = history_transaction.price[history_transaction.price["date"]==str(history_transaction_date)]

        history_price = history_price.iloc[0, -1]
        ...
        if (price_scope_min <= history_price <= price_scope_max):
            logger.debug( str(fundNum) + "比较结果：已在同一点加过")
            compare_result.append([str(history_transaction_date), str(hittory_transaction_money)])
        ...
        for i, policy in enumerate(policylist):
            ...
            if len(row) == 1:
                ...
                if warn[2] > 0:
                    ...
                    rf_target_price = round(rf_target_fund.price[rf_target_fund.price["date"] == "2021-03-05"].iloc[0].netvalue, 4)
                    
                    logger.debug(str(date) + "================ " + str(warn[0]) + "基金净值price：" + str(rf_target_price))
                    ...
                    compare_result = check_duplicate_buy(str(warn[1]), price_scope_max, price_scope_min)
